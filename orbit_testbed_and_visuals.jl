...
orbp1 = Propagators.init(Val(:TwoBody), orb1)
orbp2 = Propagators.init(Val(:TwoBody), orb2)
...
R = Instant Reward -> how much do we like the state we are currently in. We can write this as a function of the distance from the nearest point on the orbit ellipse and the distance to any known intruders.

S will not be a matrix, the state space is too big. Its also fairly simple to compute given A. Will be a Sattelite Toolbox object of type N
